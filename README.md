...
## 概述
CDN检测工具是一个插件化的网络情报分析框架，主要用于检测目标域名是否使用CDN服务，并能够识别CDN提供商。通过多维度特征分析（IP、HTTP、DNS、证书），实现高精度的CDN检测，同时支持批量检测和结果导出。

## 特性

- **多维度检测**：综合分析IP、HTTP头、CNAME、证书等多个维度的特征
- **插件化架构**：支持扩展多种插件，实现功能模块化
- **自学习能力**：根据历史检测结果和用户反馈自动调整权重
- **数据持久化**：使用SQLite数据库保存检测数据，便于历史查询和机器学习
- **批量检测**：支持单个目标和批量目标的检测
